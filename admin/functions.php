			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$result = mysqli_query($con, "SELECT title FROM events WHERE date = '".$currentDate."' AND status = 1");
...
function getEvents($date = ''){
	...
	$result = mysqli_query($con, "SELECT title FROM events WHERE date = '".$date."' AND status = 1");
...
function addEvent($date,$title){
	...
	$insert  = "INSERT INTO vents (title,date,created,modified) VALUES ('".$title."','".$date."','".$currentDate."','".$currentDate."')";
